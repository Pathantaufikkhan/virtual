...
import os
import sys
# from gui import VirtualFrameApp
...
def create_thumbnail_strip(frames, current_frame_idx, strip_height, frame_width):
    ...
    if thumbnail_strip.shape[1] < frame_width:
        ...
        thumbnail_strip = cv2.copyMakeBorder(
            thumbnail_strip, 0, 0, padding, padding, cv2.BORDER_CONSTANT
        )
...
def webcam_mode():
    ...
    while True:
        ...
        for face in faces:
            ...
            # Apply adjustments for fine-tuning
            adjusted_position = (
        ...
        elif key == ord("h"):  # Take snapshot
            snapshot_filename = f"snapshot_{snapshot_count}.png"
            cv2.imwrite(
                snapshot_filename, frame[:-thumbnail_height]
            )  # Exclude thumbnails
            snapshot_count += 1
